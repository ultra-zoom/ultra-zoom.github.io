...
class FluxControlNetMultiDiffusionPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        latent_paths = sorted(glob.glob(os.path.join(intermediate_dir, f"latent_step*.npy")))
        if len(latent_paths) > 0:
            reload_step_idx = int(latent_paths[-1].split("latent_step")[-1].split(".")[0])  # resume from this step and onwards
            latents = torch.from_numpy(np.load(latent_paths[-1])).to(device, dtype=prompt_embeds.dtype)
        else:
            reload_step_idx = 0
            ...
            latents = randn_tensor(shape, generator=generator, device=device, dtype=prompt_embeds.dtype)  # (1, 16, H//8, W//8)
            np.save(os.path.join(intermediate_dir, f"latent_step{reload_step_idx:03d}.npy"), latents.detach().cpu().to(torch.float32).numpy())  # ensures same latents on different devices
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for step_i, t in enumerate(timesteps):
                ...
                if step_i < reload_step_idx:
                    print(f"[LOG] Skipping step {step_i+1}, already computed")
                    progress_bar.update()
                    continue

                # load checkpointed chunks
                chunk_dir = os.path.join(intermediate_dir, f"latent_step{step_i+1:03d}")
                ...
                if os.path.exists(chunk_dir):
                    ...
                    if n_joint > 0:
                        try:
                            ...
                            latents_canvas = torch.from_numpy(np.load(latent_canvas_path)).to(device, dtype=prompt_embeds.dtype)
                            count_canvas = torch.from_numpy(np.load(count_canvas_path)).to(device, dtype=prompt_embeds.dtype)
                            print(f"[LOG] Step {step_i+1}, resumeing from chunk start {global_chunk_start}")
                        except:
                            print(f"[LOG] Current chunk corrupted, try previous chunk")
                            if n_joint > 1:
                ...
                if verbose:
                    print("[LOG] After initializing image/count canvas, CPU", psutil.Process().memory_info().rss / 1e9, "GB, GPU", torch.cuda.memory_allocated() / 1e9, "GB")

                for chunk_i in tqdm(range(global_chunk_start, len(coords_list[step_i]), chunk_size), desc=f"Denoising (processing in chunks)"):
                    chunk_start = chunk_i
                    ...
                    def prepare_control_latents(control_image):
                        ...
                        for h_start, w_start in tqdm(coords_chunk, desc="Preparing control latent"):
                            ...
                            control_image_pt_gpu = control_image_pt_cpu.to(device, dtype=prompt_embeds.dtype)
                        ...
                        control_latent_batch = torch.cat(control_latent_batch, dim=0)
                        control_latent_batch = (control_latent_batch - self.vae.config.shift_factor) * self.vae.config.scaling_factor

                        # pack, (1,16,128,128) --> (1,4096,64) <-- (bs, (h/2)*(w/2), c*4)
                        height_control_latent_batch, width_control_latent_batch = control_latent_batch.shape[2:]
                        control_latent_batch = self._pack_latents(
                            control_latent_batch,
                            batch_size * num_images_per_prompt * len(control_latent_batch),
                            num_channels_latents,
                            height_control_latent_batch,
                            width_control_latent_batch,
                        )
                    ...
                    if verbose:
                        print(f"[LOG] Inference step {step_i+1} out of {num_inference_steps}, step 5a, part 1: {time.time() - start_time} seconds, CPU {psutil.Process().memory_info().rss / 1e9} GB, GPU {torch.cuda.memory_allocated() / 1e9} GB")
                    ...
                    if type(control_image) == list:
                        ...
                        for cmode in control_mode:
                            if cmode is None:
                                cmode = -1
                            control_mode = torch.tensor(cmode).expand(control_latent_batch[0].shape[0]).to(device, dtype=torch.long)
                    ...
                    if verbose:
                        print(f"[LOG] Inference step {step_i+1} out of {num_inference_steps}, step 5a, part 2: {time.time() - start_time} seconds, CPU {psutil.Process().memory_info().rss / 1e9} GB, GPU {torch.cuda.memory_allocated() / 1e9} GB")
                    ...
                    if verbose:
                        print(f"[LOG] Inference step {step_i+1} out of {num_inference_steps}, step 5b: {time.time() - start_time} seconds, CPU {psutil.Process().memory_info().rss / 1e9} GB, GPU {torch.cuda.memory_allocated() / 1e9} GB")
                    ...
                    for loop_idx in tqdm(range(len(latents_batch)), desc="Processing multidiffusion windows"):
                        ...
                        if type(control_image) == list:
                            control_image_win = [control_latent_batch_[loop_idx:loop_idx+1].clone().to(device, dtype=latents_win.dtype) for control_latent_batch_ in control_latent_batch]
                        else:
                            control_image_win = control_latent_batch[loop_idx:loop_idx+1].clone()
                        ...
                        if torch.isnan(noise_pred).any():
                            print("[LOG] NaN detected in noise_pred")
                            import sys; sys.exit()
                ...
                np.save(os.path.join(intermediate_dir, f"latent_step{step_i+1:03d}.npy"), latents.detach().cpu().to(torch.float32).numpy())
                prev_path = os.path.join(intermediate_dir, f"latent_step{step_i:03d}.npy")
        ...
        if output_type == "latent":
            ...
            if decode_once:
                ...
                if verbose:
                    print("[LOG] Before initializing image/count canvas, CPU", psutil.Process().memory_info().rss / 1e9, "GB")
                ...
                if verbose:
                    print("[LOG] After initializing image/count canvas, CPU", psutil.Process().memory_info().rss / 1e9, "GB")

                # a40, 1024px, 36159/46068MB
                for chunk_i in tqdm(range(0, len(coords_list[0]), chunk_size), desc=f"Decode to pixels (processing in chunks)"):
                    chunk_start = chunk_i
                    ...
                    for i, (h_start_, w_start_) in enumerate(tqdm(coords_chunk, desc="Decoding patches")):
                        ...
                        if count < 5:
                            if label_list[0][i] == 1:
                                np2pil(self.vae.decode(latents_batch[i:i+1], return_dict=False)[0].to('cpu', dtype=torch.float32).numpy()).save(os.path.join(intermediate_dir, f"chunk{chunk_i:03d}_patch{i:03d}_hstart{h_start_}_wstart{w_start_}.png"))
                                count += 1
                    del latents_batch
                    torch.cuda.empty_cache()

                if verbose:
                    print("[LOG] After decoding patches, CPU", psutil.Process().memory_info().rss / 1e9, "GB, GPU", torch.cuda.memory_allocated() / 1e9, "GB")

                # combine with bicubic upsampled image
                for i, (h_start_, w_start_) in enumerate(tqdm(coords_list[0], desc="Alpha composite output with bicubic upsampled pixels")):
                    if label_list[0][i] == 1:
                        ...
                        torch.cuda.empty_cache()
                if verbose:
                    print("[LOG] After alpha compositing, CPU", psutil.Process().memory_info().rss / 1e9, "GB, GPU", torch.cuda.memory_allocated() / 1e9, "GB")
