...
class FluxLoraLoaderMixin(LoraBaseMixin):
    ...
    _lora_loadable_modules = ["transformer", "controlnet", "text_encoder"]
    transformer_name = TRANSFORMER_NAME
    ...
    def load_lora_weights(
        ...
        controlnet = getattr(self, self.controlnet_name) if not hasattr(self, "controlnet") else self.controlnet
        if hasattr(controlnet, "nets"):
            for i, net in enumerate(controlnet.nets):
                controlnet_lora_state_dict = {
                    k.replace(f"controlnet_{i}.", "controlnet.") : state_dict.pop(k) for k in list(state_dict.keys()) if f"controlnet_{i}" in k and "lora" in k
                }
                controlnet_norm_state_dict = {
                    k.replace(f"controlnet_{i}.", "controlnet.") : state_dict.pop(k)
                    for k in list(state_dict.keys())
                    if f"controlnet_{i}" in k and any(norm_key in k for norm_key in self._control_lora_supported_norm_keys)
                }
                has_param_with_expanded_shape = self._maybe_expand_controlnet_param_shape_or_error_(
                    net, controlnet_lora_state_dict, controlnet_norm_state_dict
                )

                if has_param_with_expanded_shape:
                    logger.info(
                        "The LoRA weights contain parameters that have different shapes that expected by the controlnet. "
                        "As a result, the state_dict of the controlnet has been expanded to match the LoRA parameter shapes. "
                        "To get a comprehensive list of parameter names that were modified, enable debug logging."
                    )
                controlnet_lora_state_dict = self._maybe_expand_lora_state_dict(
                    transformer=net, lora_state_dict=controlnet_lora_state_dict
                )
                ...
                if len(controlnet_norm_state_dict) > 0:
                    net._controlnet_norm_layers = self._load_norm_into_controlnet(
                        controlnet_norm_state_dict,
                        controlnet=net,
                        discard_original_layers=False,
                    )
            ...
            controlnet_lora_state_dict = {
                k: state_dict.pop(k) for k in list(state_dict.keys()) if "controlnet." in k and "lora" in k
            }
            controlnet_norm_state_dict = {
                k: state_dict.pop(k)
                for k in list(state_dict.keys())
                if "controlnet." in k and any(norm_key in k for norm_key in self._control_lora_supported_norm_keys)
            }
            has_param_with_expanded_shape = self._maybe_expand_controlnet_param_shape_or_error_(
                controlnet, controlnet_lora_state_dict, controlnet_norm_state_dict
            )
            ...
            controlnet_lora_state_dict = self._maybe_expand_lora_state_dict(
                transformer=controlnet, lora_state_dict=controlnet_lora_state_dict
            )
            ...
            if len(controlnet_norm_state_dict) > 0:
                controlnet._controlnet_norm_layers = self._load_norm_into_controlnet(
                    controlnet_norm_state_dict,
                    controlnet=controlnet,
                    discard_original_layers=False,
                )
    ...
    def load_lora_into_controlnet(
        ...
        Parameters:
            ...
            network_alphas (`Dict[str, float]`):
                ...
                same meaning as the `--network_alpha` option in the kohya-ss trainer script. Refer to [this
                link](https://github.com/darkstorm2150/sd-scripts/blob/main/docs/train_network_README-en.md#execute-learning).
            controlnet (`FluxTransformer2DModel`):
                The Transformer model to load the LoRA layers into.
            adapter_name (`str`, *optional*):
                Adapter name to be used for referencing the loaded adapter model. If not specified, it will use
                `default_{i}` where i is the total number of adapters being loaded.
        ...
        keys = list(state_dict.keys())
        controlnet_present = any(key.startswith(cls.controlnet_name) for key in keys)
...
# for n, p in controlnet.named_modules():
#     print(n)
    ...
    def save_lora_weights(
        ...
        controlnet_lora_layers: Union[Dict[str, Union[torch.nn.Module, torch.Tensor]], List[Dict[str, Union[torch.nn.Module, torch.Tensor]]]] = None,
        text_encoder_lora_layers: Dict[str, torch.nn.Module] = None,
        is_main_process: bool = True,
        weight_name: str = None,
        save_function: Callable = None,
        ...
        if controlnet_lora_layers:
            if isinstance(controlnet_lora_layers, dict):
                state_dict.update(cls.pack_weights(controlnet_lora_layers, cls.controlnet_name))
            elif isinstance(controlnet_lora_layers, list):
                for i, controlnet_lora_layer in enumerate(controlnet_lora_layers):
                    state_dict.update(cls.pack_weights(controlnet_lora_layer, f"{cls.controlnet_name}_{i}"))
    ...
    @classmethod
    def _maybe_expand_controlnet_param_shape_or_error_(
        cls,
        ...
        # Expand controlnet parameter shapes if they don't match lora
        has_param_with_shape_update = False
        overwritten_params = {}

        is_peft_loaded = getattr(controlnet, "peft_config", None) is not None
        for name, module in controlnet.named_modules():
            if isinstance(module, torch.nn.Linear):
                ...
                # `bitsandbytes`, for example, flatten the weights when using 4bit. 8bit bnb models
                # preserve weight shape.
                module_weight_shape = cls._calculate_module_shape(model=controlnet, base_module=module)
                ...
                if out_features > module_out_features or in_features > module_in_features:
                    has_param_with_shape_update = True
                    parent_module_name, _, current_module_name = name.rpartition(".")
                    parent_module = controlnet.get_submodule(parent_module_name)
                    ...
                    if current_module_name in _MODULE_NAME_TO_ATTRIBUTE_MAP_FLUX:
                        attribute_name = _MODULE_NAME_TO_ATTRIBUTE_MAP_FLUX[current_module_name]
                        new_value = int(expanded_module.weight.data.shape[1])
                        old_value = getattr(controlnet.config, attribute_name)
                        setattr(controlnet.config, attribute_name, new_value)
                        logger.info(
                            f"Set the {attribute_name} attribute of the model to {new_value} from {old_value}."
                        )
        ...
        if len(overwritten_params) > 0:
            controlnet._overwritten_params = overwritten_params
